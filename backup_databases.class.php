...
class backup_databases {
  ...
  public function backup_database (pdo &$db, $tables_to_backup = '*', $structure_only_tables = array()): bool {
    ...
    $return .= '--' . $this->nl . $this->nl . $this->nl;                                          // create a header for the SQL file
    ...
    for ($table_idx = 0; $table_idx < count($tables_to_backup); $table_idx++) {                   // cycle through all of the tables
      ...
      $return .= $this->nl . $this->nl . $createtable . ';';                                      // add create table string -- this is all that is needed for struction only tables
  ...
  private function write_file (string $dbname, string &$return): bool {
    ...
    $archive_info_filename = 'info.txt';
    ...
    if (file_exists($dirname . $archive_info_filename))
      $lastmd5 = file($dirname . $archive_info_filename)[1];                                      // get last backup's MD5
    ...
    if ($lastmd5 != $md5val) {
      ...
      $handle = fopen($dirname . $archive_info_filename, 'w+');                                   // open info file
